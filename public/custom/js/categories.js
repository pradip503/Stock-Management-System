	...
	if(categoriesId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#editCategoriesStatus").val(response.data.categories_active);
				// add the categories id 
				$(".editCategoriesFooter").after('<input type="hidden" name="editCategoriesId" id="editCategoriesId" value="'+response.data.categories_id+'" />');
...
function loadCategories(){
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      $('#manageCategoriesTableBody').html('');
      
      data.forEach((category,i) => {

        if(category.categories_active == 1) {
          // activate member
		...
		<div class="btn-group">
		  ...
		  <ul class="dropdown-menu">
			<li><a type="button" data-toggle="modal" id="editCategoriesModalBtn" data-target="#editCategoriesModal" onclick="editCategories('${category.categories_id}')"> <i class="glyphicon glyphicon-edit"></i> Edit</a></li>
			<li><a type="button" data-toggle="modal" data-target="#removeCategoriesModal" id="removeCategoriesModalBtn" onclick="removeCategories('${category.categories_id}')"> <i class="glyphicon glyphicon-trash"></i> Remove</a></li>       
		  </ul>
		</div>`;
  
        $('#manageCategoriesTableBody').append(`
        <tr>
        <td>${i+1}</td>
          <td>${category.categories_name}</td>
          <td>`+ $activeCategories +`</td>
          <td>` + $button +`</td>
        </tr>
      `);
      });
