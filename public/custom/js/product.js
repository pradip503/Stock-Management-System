...
$(document).ready(function () {
	...
	$("#addProductModalBtn").unbind('click').bind('click', function () {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function () {
			...
			if (unitName == "") {
				$("#unitName").after('<p class="text-danger">Unit field is required</p>');
			...
			if (productName && unitName && quantity && rate && brandName && categoryName && productStatus) {
	...
	if (productId) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				//product unit
				$("#editUnitName").val(response.data.product_unit);
				// category name
				...
				$("#editProductForm").unbind('submit').bind('submit', function () {
					...
					if (unitName == "") {
						$("#editUnitName").after('<p class="text-danger">Unit field is required</p>');
...
//fetch brand options before adding product
...
//fetch unit optioin before adding product
function fetchUnits() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			data.forEach(unit => {
				$unitOptions = $unitOptions + `<option value='${unit.unit_short_name}'>${unit.unit_short_name}</option>`
			});

			$('#unitName').append(`<option value="">~~SELECT~~</option>` + $unitOptions);

		},
		error: function (error) {
			$('#unitName').html(`Unable to fetch units!`);
		}
	});
}
...
//fetch unit optioin before editing product
function fetchEditUnits() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			$('#editUnitName').append(`<option value="">~~SELECT~~</option>` + $unitOptions);

		},
		error: function (error) {
			$('#editUnitName').html(`Unable to fetch units!`);
		}
	});
}
