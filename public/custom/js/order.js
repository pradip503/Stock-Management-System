...
$(document).ready(function () {
	...
	if (divRequest == 'add') {
		...
		// loads order in manaOrder Window
		loadOrders();
					...
					ordersArray.forEach((order, i) => {
						...
						<div class="btn-group">
						  ...
						  <ul class="dropdown-menu">
							<li><a href="order?o=editOrd&i=${order.order_id}" id="editOrderModalBtn"> <i class="glyphicon glyphicon-edit"></i> Edit</a></li>
	...
	$.ajax({
		...
		success: function (response) {
			...
			response.forEach(product => {
				tr += '<option value="' + product.product_id + '">' + product.product_name + '</option>';
			});
		...
		if (productId == "") {
			...
			$.ajax({
				...
				success: function (response) {
					...
					$("#rate" + row).val(response.data.rate);
					$("#rateValue" + row).val(response.data.rate);

					$("#quantity" + row).val(1);

					var total = Number(response.data.rate) * 1;
